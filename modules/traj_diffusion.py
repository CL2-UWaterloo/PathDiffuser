# This code is based on Optimizing Diffusion Models for Joint Trajectory Prediction and Controllable Generation, 
# Copyright (c) 2023, Zikang Zhou. All rights reserved.
# Modifications Copyright (c)  Da Saem Lee, 2025
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
import math
from typing import Dict, Mapping
from pynvml import *
nvmlInit()

import torch
import torch.nn as nn
from torch_cluster import radius
from torch_cluster import radius_graph
from torch_geometric.data import Batch
from torch_geometric.data import HeteroData
import copy

from layers import AttentionLayer
from layers import FourierEmbedding
from utils import angle_between_2d_vectors
from utils import weight_init
from utils import wrap_angle
import numpy as np

import warnings
warnings.filterwarnings('ignore', category=UserWarning, message='TypedStorage is deprecated')

def rot_angle_2d(mat):
    """
    Computes the rotation angle (in radians) for a batch of 2D rotation matrices.
    mat: shape (..., 2, 2)
    """
    eps = 1e-4
    # The trace is the sum of the diagonal elements
    cos = (mat[..., 0, 0] + mat[..., 1, 1]) / 2
    # Clamp to avoid numerical issues with acos
    cos = cos.clamp(-1 + eps, 1 - eps)
    angle = torch.acos(cos)
    return angle

class TrajDiffusion(nn.Module):

    def __init__(self, args):
        super().__init__()
        self.diff_type = args.diff_type
        self.net = TrajDenoiser(
            dataset=args.dataset,
            input_dim=args.input_dim,
            hidden_dim=args.hidden_dim,
            output_dim=args.output_dim,
            output_head=args.output_head,
            init_timestep=args.init_timestep,
            num_generation_timestep=args.num_generation_timestep,
            num_freq_bands=args.num_freq_bands,
            num_layers=args.num_denoiser_layers,
            num_heads=args.num_heads,
            head_dim=args.head_dim,
            dropout=args.dropout,
            diff_type=args.diff_type,
            m_dim = args.m_dim
        )
        
        self.var_sched = VarianceSchedule(
                num_steps = args.num_diffusion_steps,
                beta_1 = args.beta_1,
                beta_T = args.beta_T,
                mode = 'linear'
            )
        
    def get_loss(self, 
                 m,
                 data: HeteroData,
                 scene_enc: Mapping[str, torch.Tensor],
                 eval_mask = None,
                 clean_data = None,
                 num_samples = None) -> Dict[str, torch.Tensor]:
        
        return self.get_loss_vd(m,data,scene_enc,eval_mask,clean_data, num_samples)

    def get_loss_vd(self, 
                 m,
                 data: HeteroData,
                 scene_enc: Mapping[str, torch.Tensor],
                 eval_mask = None,
                 clean_data= None,
                 num_samples = 1) -> Dict[str, torch.Tensor]:
        # m: [num_agents, d_latent]
        m = m.unsqueeze(1).repeat(1,num_samples,1)
        x_0 = m
        device = m.device
        batch_idx = data['agent']['batch'][eval_mask]
        
        num_agents = eval_mask.sum()# mean.size(0)
        num_dim = self.net.m_dim
        num_scenes = batch_idx[-1].item()+1
        
        t = torch.tensor(self.var_sched.uniform_sample_t(num_scenes)).to(device).unsqueeze(0).repeat(num_agents,1)
        t = t[torch.arange(num_agents), batch_idx]

        alpha_bar = self.var_sched.alpha_bars[t][:,None].to(device)
        beta = self.var_sched.betas[t][:,None].to(device)
        
        c0 = torch.sqrt(alpha_bar).unsqueeze(-1).repeat(1,num_samples, 1)
        c1 = torch.sqrt(1 - alpha_bar).unsqueeze(-1).repeat(1,num_samples, 1)
        
        e_rand = torch.randn_like(x_0).to(device)
        
        x_t = c0 * x_0 + c1 * e_rand
        
        # now delta_rot_pred is angle! add the ego initial angle, then we can get the heading relative to its own
        g_theta = self.net(x_t, beta, data, scene_enc, num_samples = num_samples, eval_mask=eval_mask, clean_data = clean_data)

        loss = ((e_rand- g_theta) ** 2).mean()
        x_0_reconstructed = (x_t - c1 * g_theta) / c0

        return loss, x_0_reconstructed

    def sample(self, 
               num_samples: int,
               data: HeteroData,
               scene_enc: Mapping[str, torch.Tensor],
               if_output_diffusion_process = False,
               start_data = None,
               reverse_steps = None,
               eval_mask = None,
               sampling="ddpm", 
               stride=20,
               grad_guid = None,
               cond_gen = None,
               guid_param = None,
               uc = None,
               clean_data = None
               ) -> Dict[str, torch.Tensor]:
        if grad_guid != None:
            return self.sample_guide(num_samples, data, scene_enc, if_output_diffusion_process, start_data,reverse_steps,
                                eval_mask, sampling, stride, clean_data, grad_guid=grad_guid, cond_gen=cond_gen, guid_param=guid_param)
        else:
            return self.sample_vd(num_samples, data, scene_enc, if_output_diffusion_process, start_data,reverse_steps,
                                eval_mask, sampling, stride, clean_data)
    def lat2traj(self, latent, V_k, s_mean, num_samples):
        return torch.matmul(latent, V_k.unsqueeze(0).repeat(num_samples,1,1)) + s_mean.unsqueeze(0)
    
            
    def task_diff(self, task, traj, label):
        if 'goal' in task:
            goal_pt = label
            if 'goal_at5s' in task:
                index = 50
            else:
                index = 59
                
            goal_diff = ((traj[:,:,index,:] - goal_pt.unsqueeze(1))**2)

            goal_diff[...,1] = goal_diff[...,1] * 1
            goal_diff = goal_diff.mean(-1)
            return goal_diff
            
    def sample_vd(self, 
               num_samples: int,
               data: HeteroData,
               scene_enc: Mapping[str, torch.Tensor],
               if_output_diffusion_process = False,
               start_data = None,
               reverse_steps = None,
               eval_mask = None,
               sampling="ddpm", 
               stride=20,
               clean_data = None
               ) -> Dict[str, torch.Tensor]:
        
        if reverse_steps is None:
            reverse_steps = self.var_sched.num_steps
            
        device = scene_enc['x_pl'].device

        num_agents = eval_mask.sum()# mean.size(0)
        num_dim = self.net.m_dim

        e_rand = torch.randn([num_agents, num_samples, num_dim]).to(device)

        if start_data == None:
            x_T = e_rand
        else:
            c0 = torch.sqrt(self.var_sched.alpha_bars[reverse_steps]).to(device)
            c1 = torch.sqrt(1-self.var_sched.alpha_bars[reverse_steps]).to(device)
            x_T = c0 * start_data.unsqueeze(1) + c1 * e_rand
            
            
        x_t_list = [x_T]
        torch.cuda.empty_cache()
            
        for t in range(reverse_steps, 0, -stride):
            z = torch.randn_like(x_T) if t > 1 else torch.zeros_like(x_T)

            beta = self.var_sched.betas[t]
            
            alpha = self.var_sched.alphas[t]    
            alpha_bar = self.var_sched.alpha_bars[t]
            alpha_bar_next = self.var_sched.alpha_bars[t-stride]
            c0 = 1 / torch.sqrt(alpha)
            c1 = (1-alpha) / torch.sqrt(1 - alpha_bar)
            sigma = self.var_sched.get_sigmas(t, 0)
            
            x_t = x_t_list[-1]
            
            with torch.no_grad():
                beta = beta.to(device).repeat(num_agents * num_samples).unsqueeze(-1)
                g_theta = self.net(copy.deepcopy(x_t), beta, data, scene_enc, num_samples = num_samples, eval_mask=eval_mask, clean_data = clean_data)
                
            if sampling == 'ddpm':
                x_next = c0 * (x_t - c1 * g_theta) + sigma * z
            elif sampling == 'ddim':
                x0_t = (x_t - g_theta * (1 - alpha_bar).sqrt()) / alpha_bar.sqrt()
                x_next = alpha_bar_next.sqrt() * x0_t + (1 - alpha_bar_next).sqrt() * g_theta
                
            
            if True in torch.isnan(x_next):
                print('nan:',t)
            x_t_list.append(x_next.detach())
            if not if_output_diffusion_process:
                x_t_list.pop(0)
            
        if if_output_diffusion_process:
            return x_t_list
        else:
            return x_t_list[-1]
            

    
    def sample_guide(self, 
               num_samples: int,
               data: HeteroData,
               scene_enc: Mapping[str, torch.Tensor],
               if_output_diffusion_process = False,
               start_data = None,
               reverse_steps = None,
               eval_mask = None,
               sampling="ddpm", 
               stride=20,
               grad_guid = None,
               cond_gen = None,
               guid_param = None,
               ) -> Dict[str, torch.Tensor]:
        
        if grad_guid != None:
            task = guid_param['task']
            guid_method = guid_param['guid_method']
            cost_param_costl = guid_param['cost_param']['cost_param_costl']
            cost_param_threl = guid_param['cost_param']['cost_param_threl']
            [guid_label, s_mean, V_k, VT_k, latent_mean, latent_std] = grad_guid
        
        if reverse_steps is None:
            reverse_steps = self.var_sched.num_steps
        
        device = s_mean.device
        num_agents = eval_mask.sum()# mean.size(0)
        num_dim = self.net.m_dim
        
        # std = torch.ones_like(std)
        e_rand = torch.randn([num_agents, num_samples, num_dim]).to(device) 

        s_T = torch.sqrt(self.var_sched.alpha_bars[reverse_steps].to(device))
        if start_data == None:
            c1 = 1
            x_T = c1 * e_rand + s_T
        else:
            c0 = torch.sqrt(self.var_sched.alpha_bars[reverse_steps]).to(device)
            c1 = torch.sqrt(1-self.var_sched.alpha_bars[reverse_steps]).to(device)

            if start_data.dim() == 2:
                x_T = c0 * start_data.unsqueeze(1) + c1 * e_rand
            elif start_data.dim() == 3:
                x_T = c0 * start_data + c1 * e_rand
            
        x_t_list = [x_T]
        torch.cuda.empty_cache()
        
        for t in range(reverse_steps, 0, -stride):

            z = torch.randn_like(x_T) if t > 1 else torch.zeros_like(x_T)
            beta = self.var_sched.betas[t]
            alpha = self.var_sched.alphas[t]
            alpha_bar = self.var_sched.alpha_bars[t]
            alpha_bar_next = self.var_sched.alpha_bars[t-stride]
            c0 = 1 / torch.sqrt(alpha)
            c1 = (1-alpha) / torch.sqrt(1 - alpha_bar)
            
            x_t = x_t_list[-1]
        
            [idx, target_mode] = cond_gen
            x_t[idx,:,:] = target_mode.unsqueeze(0).repeat(num_samples,1)
        
            if guid_method == 'ECM':       
                    with torch.no_grad():
                        beta_emb = beta.to(device).repeat(num_agents * num_samples).unsqueeze(-1)
                        g_theta = self.net(copy.deepcopy(x_t), beta_emb, data, scene_enc, num_samples = num_samples, eval_mask=eval_mask)
                        
                    with torch.inference_mode(False):
                        temp_e = g_theta.clone().detach().float()
                        temp_x_t = x_t.clone().detach().float()
                        temp_x_0 = (temp_x_t - torch.sqrt(1-alpha_bar).clone() * temp_e) / torch.sqrt(alpha_bar).clone()
                        temp_x_0.requires_grad = True
                        temp_x_0_unnor = temp_x_0 * (latent_std.unsqueeze(0).unsqueeze(0)+0.1) + latent_mean.unsqueeze(0).unsqueeze(0)
                        rec_traj = torch.matmul(temp_x_0_unnor.permute(1,0,2), V_k.unsqueeze(0).repeat(num_samples,1,1)) + s_mean.unsqueeze(0)
                        rec_traj = rec_traj.permute(1,0,2)
                        rec_traj = rec_traj.view(rec_traj.size(0), rec_traj.size(1),60,2)
                        diff = self.task_diff(task, rec_traj, guid_label)
                        error = diff.mean()
                        error.backward()
                        # note it is the gradient of x0
                        grad = temp_x_0.grad
                        
                    grad = grad * cost_param_costl
                    scale = 1 * cost_param_threl
                    grad = torch.clip(grad, min = -scale, max = scale)
                    m_0 = temp_x_0 - grad
                    
                    # g_theta = torch.randn_like(x_T) * std
                    x_next = alpha_bar_next.sqrt() * m_0 + (1 - alpha_bar_next).sqrt() * g_theta

            if True in torch.isnan(x_next):
                print('nan:',t)
            x_t_list.append(x_next.detach())
            if not if_output_diffusion_process:
                x_t_list.pop(0)
            
        if if_output_diffusion_process:
            return x_t_list
        else:
            return x_t_list[-1]


class TrajDenoiser(nn.Module):

    def __init__(self,
                 dataset: str,
                 input_dim: int,
                 hidden_dim: int,
                 output_dim: int,
                 output_head: bool,
                 init_timestep: int,
                 num_generation_timestep: int,
                 num_freq_bands: int,
                 num_layers: int,
                 num_heads: int,
                 head_dim: int,
                 dropout: float,
                 diff_type: str,
                 m_dim: int) -> None:
        super(TrajDenoiser, self).__init__()
        self.dataset = dataset
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
        self.output_head = output_head
        self.init_timestep = init_timestep
        self.num_generation_timestep = num_generation_timestep
        self.num_layers = num_layers
        self.num_heads = num_heads
        self.head_dim = head_dim
        self.dropout = dropout
        self.diff_type = diff_type
        self.m_dim = m_dim
        
        input_dim_r_pl2m = 3
        input_dim_r_a2m = 3
        input_dim_r_c2m = 1
        
        self.proj_in = nn.Linear(self.m_dim, self.hidden_dim)
        
        self.proj_in_2 = nn.Sequential(
            nn.LayerNorm(hidden_dim),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_dim, hidden_dim),
        )

        self.proj_out = nn.Linear(self.hidden_dim, self.m_dim)
        
        self.r_pl2m_emb = FourierEmbedding(input_dim=input_dim_r_pl2m, hidden_dim=hidden_dim,
                                           num_freq_bands=num_freq_bands)
        self.r_a2m_emb = FourierEmbedding(input_dim=input_dim_r_a2m, hidden_dim=hidden_dim,
                                          num_freq_bands=num_freq_bands)
        
        self.r_c2m_emb = FourierEmbedding(input_dim=input_dim_r_c2m, hidden_dim=hidden_dim,
                                          num_freq_bands=num_freq_bands)
        
        noise_dim = 1
        self.noise_emb = FourierEmbedding(input_dim=noise_dim, hidden_dim=hidden_dim,
                                          num_freq_bands=num_freq_bands)
        
        self.type_a_emb = nn.Embedding(10, hidden_dim)
        self.max_speed = 0
        self.speed_emb = nn.Embedding(30, hidden_dim)
        ########
        
        self.proj_cand = nn.Linear(self.m_dim, self.hidden_dim)
        
        self.cand2m_layers = nn.ModuleList(
             [AttentionLayer(hidden_dim=hidden_dim, num_heads=num_heads, head_dim=head_dim, dropout=dropout,
                             bipartite=True, has_pos_emb=True) for _ in range(num_layers)]
         )
        self.pl2m_attn_layers = nn.ModuleList(
            [AttentionLayer(hidden_dim=hidden_dim, num_heads=num_heads, head_dim=head_dim, dropout=dropout,
                            bipartite=True, has_pos_emb=True) for _ in range(num_layers)]
        )
        self.m2m_attn_layer = nn.ModuleList(
            [AttentionLayer(hidden_dim=hidden_dim, num_heads=num_heads, head_dim=head_dim, dropout=dropout,
                            bipartite=True, has_pos_emb=True) for _ in range(num_layers)]
        )
        self.to_out = SkipMLP(d_model=hidden_dim)
        
        self.apply(weight_init)
    
    def forward(self,
                m,
                beta,
                data: HeteroData,
                scene_enc: Mapping[str, torch.Tensor],
                num_samples: int,
                eval_mask,
                clean_data) -> Dict[str, torch.Tensor]:
        device = m.device
        
        self.num_samples = num_samples
        
        batch_idx = data['agent']['batch'][eval_mask]
        batch_size = data.num_graphs

        
        x_pl = scene_enc['x_pl'].repeat(self.num_samples, 1)
        # num_agents x 128
        categorical_embs_m = [
                self.type_a_emb(data['agent']['type'][eval_mask].long()),
            ]
        
        mask_src = data['agent']['valid_mask'][eval_mask, self.init_timestep:].contiguous()
        mask_dst = mask_src.clone().repeat(1, self.num_samples)

        pos_pl = data['map_polygon']['position'][:, :self.input_dim]
        orient_pl = data['map_polygon']['orientation']
        
        init_pos_all = data['agent']['position'][eval_mask][:, self.init_timestep, :2]
        init_heading_all = data['agent']['heading'][eval_mask][:, self.init_timestep]
        init_heading_vector_all = torch.stack([init_heading_all.cos(), init_heading_all.sin()], dim=-1)
        
        init_velocity = data['agent']['velocity'][eval_mask][:, self.init_timestep, :]
        init_speed = torch.norm(init_velocity, p=2, dim=1)
        init_speed =  torch.maximum(init_speed, torch.zeros_like(init_speed))
        init_speed = torch.minimum(init_speed, 29 * torch.ones_like(init_speed))
        init_speed_emb = self.speed_emb((init_speed).long())
        
        # projected candidate features
        candidates = clean_data['primitives']
    
        candidate_emb = self.proj_cand(candidates)
        ############ MAP
        # RPE for all agents initial state to map polygons
        agent_batch = data['agent']['batch'][eval_mask]
        edge_index_pl2m = radius(
            x=init_pos_all[:, :2],
            y=pos_pl[:, :2],
            r=torch.inf,
            batch_x=agent_batch if isinstance(data, Batch) else None,
            batch_y=data['map_polygon']['batch'] if isinstance(data, Batch) else None,
            max_num_neighbors=300)
        
        edge_index_pl2m = edge_index_pl2m[:, mask_dst[edge_index_pl2m[1], 0]]
        rel_pos_pl2m = pos_pl[edge_index_pl2m[0]] - init_pos_all[edge_index_pl2m[1]]
        rel_orient_pl2m = wrap_angle(orient_pl[edge_index_pl2m[0]] - init_heading_all[edge_index_pl2m[1]])
        r_pl2m = torch.stack(
            [torch.norm(rel_pos_pl2m[:, :2], p=2, dim=-1),
             angle_between_2d_vectors(ctr_vector=init_heading_vector_all[edge_index_pl2m[1]], nbr_vector=rel_pos_pl2m[:, :2]),
             rel_orient_pl2m], dim=-1)
        r_pl2m = self.r_pl2m_emb(continuous_inputs=r_pl2m, categorical_embs=None)
        edge_index_pl2m = torch.cat([edge_index_pl2m + i * edge_index_pl2m.new_tensor(
            [[data['map_polygon']['num_nodes']], [init_pos_all.size(0)]]) for i in range(self.num_samples)], dim=1)
        r_pl2m = r_pl2m.repeat(self.num_samples, 1)
    
        # RPE from agents to agents
        edge_index_a2m = radius_graph(
            x=init_pos_all[:, :2],
            r=torch.inf,
            batch=agent_batch if isinstance(data, Batch) else None,
            loop=False,
            max_num_neighbors=300)
        edge_index_a2m = edge_index_a2m[:, mask_src[:, -1][edge_index_a2m[0]] & mask_dst[edge_index_a2m[1], 0]]
        
        rel_pos_a2m = init_pos_all[edge_index_a2m[0]] - init_pos_all[edge_index_a2m[1]]
        rel_head_a2m = wrap_angle(init_heading_all[edge_index_a2m[0]] - init_heading_all[edge_index_a2m[1]])
        
        r_a2m = torch.stack(
            [torch.norm(rel_pos_a2m[:, :2], p=2, dim=-1),
             angle_between_2d_vectors(ctr_vector=init_heading_vector_all[edge_index_a2m[1]], nbr_vector=rel_pos_a2m[:, :2]),
             rel_head_a2m], dim=-1)
        r_a2m = self.r_a2m_emb(continuous_inputs=r_a2m, categorical_embs=None)
        edge_index_a2m = torch.cat(
            [edge_index_a2m + i * edge_index_a2m.new_tensor([init_pos_all.size(0)]) for i in
             range(self.num_samples)], dim=1)
        r_a2m = r_a2m.repeat(self.num_samples, 1)


        # ###############################################

        beta_emb = self.noise_emb(beta)
        beta_emb = beta_emb.repeat(self.num_samples, 1)

        m = self.proj_in(m).view(-1, self.hidden_dim)
        m = m + categorical_embs_m[0] + init_speed_emb
        m = self.proj_in_2(m)

        m_sum = m
        
        cand_heading = data['agent']['candidate_orientation']

        num_agents_batch = torch.zeros((batch_size+1), device=device, dtype=torch.int32)
        num_agents_batch[1:] = data['agent']['batch'][eval_mask].bincount(minlength=batch_size).cumsum(dim=0)

        num_candidates = torch.zeros((batch_size+1), device=device, dtype=torch.int32)
        num_candidates[1:] = data['agent']['num_candidates'].cumsum(dim=0)

        edge_candidate_idx = data['agent']['candidate_agent_index'].clone()
        for i in range(batch_size):
            edge_candidate_idx [ num_candidates[i]:num_candidates[i+1]] += num_agents_batch[i]

        edge_index_can2m = torch.stack([torch.arange(candidate_emb.size(0), device=device),
                                         edge_candidate_idx], dim=0)
        
        r_c2m = wrap_angle(cand_heading[edge_index_can2m[0]] - init_heading_all[edge_index_can2m[1]]).unsqueeze(-1)
        r_c2m = self.r_c2m_emb(continuous_inputs=r_c2m, categorical_embs=None)
        
        for i in range(self.num_layers):
            m = m + beta_emb

            # relativeness from map
            m = self.pl2m_attn_layers[i]((x_pl, m), r_pl2m, edge_index_pl2m)
            # social each other
            m = self.m2m_attn_layer[i]((m, m), r_a2m, edge_index_a2m)

            m = self.cand2m_layers[i]((candidate_emb, m), r_c2m, edge_index_can2m)
            m_sum += m # skip connection
            
        out = self.to_out(m_sum)
        out = out.view(-1, self.num_samples, self.hidden_dim)
        
        
        return self.proj_out(out)
    
        
class MyMLP(torch.nn.Module):
    def __init__(self, d_in = 128, d_h = 128, d_out = 128, act_layer=nn.GELU, drop=0.):
        super().__init__()
        self.linear_in = torch.nn.Linear(d_in, d_h)
        self.ac = act_layer()
        self.linear_out = torch.nn.Linear(d_h, d_out)
        self.drop = nn.Dropout(drop)

    def forward(self, x):          
        out = self.linear_in(x)
        out = self.ac(out)
        out = self.drop(out)
        out = self.linear_out(out)
        return out


class VarianceSchedule(nn.Module):

    def __init__(self, num_steps, mode='linear',beta_1=1e-4, beta_T=5e-2,cosine_s=8e-3):
        super().__init__()
        assert mode in ('linear', 'cosine')
        self.num_steps = num_steps
        self.beta_1 = beta_1
        self.beta_T = beta_T
        self.mode = mode

        if mode == 'linear':
            betas = torch.linspace(beta_1, beta_T, steps=num_steps)
        elif mode == 'cosine':
            timesteps = (
            torch.arange(num_steps + 1) / num_steps + cosine_s
            )
            alphas = timesteps / (1 + cosine_s) * math.pi / 2
            alphas = torch.cos(alphas).pow(2)
            alphas = alphas / alphas[0]
            betas = 1 - alphas[1:] / alphas[:-1]
            betas = betas.clamp(max=0.999)

        betas = torch.cat([torch.zeros([1]), betas], dim=0)     # Padding
        
        alphas = 1 - betas
        
        log_alphas = torch.log(alphas)
        for i in range(1, log_alphas.size(0)):  # 1 to T
            log_alphas[i] += log_alphas[i - 1]
        alpha_bars = log_alphas.exp()
        # print('alpha_bars',alpha_bars)
        # print('beta / sqrt 1 - alpha_bars',betas/torch.sqrt(1-alpha_bars))
        # print('1/ sqrt 1 - alpha_bars',1/torch.sqrt(1-alpha_bars))
        # print(' 1 - alpha_bars',(1-alpha_bars[:-1])/(1-alpha_bars[1:]))
        # exit()
        sigmas_flex = torch.sqrt(betas)
        sigmas_inflex = torch.zeros_like(sigmas_flex)
        for i in range(1, sigmas_flex.size(0)):
            sigmas_inflex[i] = ((1 - alpha_bars[i-1]) / (1 - alpha_bars[i])) * betas[i]
        sigmas_inflex = torch.sqrt(sigmas_inflex)

        self.register_buffer('betas', betas)
        self.register_buffer('alphas', alphas)
        self.register_buffer('alpha_bars', alpha_bars)
        self.register_buffer('sigmas_flex', sigmas_flex)
        self.register_buffer('sigmas_inflex', sigmas_inflex)
        
        # kt
        sqrt_alpha_bars = torch.sqrt(alpha_bars)
        # kt = sqrt_alpha_bars * (1 - sqrt_alpha_bars) # quadratic-shift
        # kt = torch.linspace(0, 0, steps=num_steps+1)
        kt = 1 - sqrt_alpha_bars # shifted diffusion
        # print('kt', kt)
        # exit()
        # kt[0] = 0
        self.register_buffer('kt', kt)
        
        inv_sqrt_alpha = 1 / torch.sqrt(alphas)
        co_g = betas / torch.sqrt(1-alpha_bars)
        co_st = torch.sqrt(alphas[1:]) * (1-alpha_bars[:-1])/(1-alpha_bars[1:])
        co_st = torch.cat([torch.tensor([0]),co_st])
        co_z = torch.sqrt((1-alpha_bars[:-1])/(1-alpha_bars[1:])*betas[1:])
        co_z = torch.cat([torch.tensor([0]),co_z])
        self.register_buffer('inv_sqrt_alpha', inv_sqrt_alpha)
        self.register_buffer('co_g', co_g)
        self.register_buffer('co_st', co_st)
        self.register_buffer('co_z', co_z)
        

    def uniform_sample_t(self, batch_size):
        ts = np.random.choice(np.arange(1, self.num_steps+1), batch_size)
        return ts.tolist()

    def get_sigmas(self, t, flexibility):
        assert 0 <= flexibility and flexibility <= 1
        sigmas = self.sigmas_flex[t] * flexibility + self.sigmas_inflex[t] * (1 - flexibility)
        return sigmas
    
class SkipMLP(torch.nn.Module):
    def __init__(self, d_model = 128, act_layer=nn.GELU):
        super().__init__()
        self.linear = torch.nn.Linear(d_model, d_model)
        self.ac = act_layer()
        self.norm1 = nn.LayerNorm([d_model])
        self.norm2 = nn.LayerNorm([d_model])

    def forward(self, x):          
        out = x + self.ac(self.linear(x))
        out = self.norm2(x + self.norm1(out))
        return out
